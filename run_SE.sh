...
#SBATCH -D /data/astrolab/Carlson/Astro/VLASS_Data/Rnd2/J0000+0000 # Working directory (PBS_O_WORKDIR) set to your Lustre area
#SBATCH --mem=64GB
...
python3 andromeda_script_gen.py J0000+0000 


# casa's python requires a DISPLAY for matplot, so create a virtual X server
xvfb-run -a casa --pipeline --nogui --nologger -c working/command_script.py